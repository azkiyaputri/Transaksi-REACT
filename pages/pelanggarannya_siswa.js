...
class PelanggarannyaSiswa extends React.Component {
    constructor() {  
        super();  
        this.state = {
            token: "",
            siswa: [],
            pelanggaran: [],
            pelanggarannya_siswa: [],
            detail_pelanggaran_siswa: [],
            list_pelanggaran: [],
            id_pelanggarannya_siswa: "",
            id_pelanggaran: "",
            isModal1Open: false,
            isModal2Open: false,
            isModal3Open: false
    ...
    getPelanggarannyaSiswa = () => {
        let url = "http://localhost:2000/pelanggarannya_siswa";
        // mengakses api untuk mengambil data pelanggaran
        axios.get(url,this.headerConfig())
        .then(response => {
          // mengisikan data dari respon API ke array pelanggaran
          this.setState({pelanggarannya_siswa: response.data.pelanggarannya_siswa});
        })
        .catch(error => {
          console.log(error);
        });
    ...
    handleDetail = (id_pelanggarannya_siswa) => {
        let url = "http://localhost:2000/detail_pelanggaran_siswa/ " + id_pelanggarannya_siswa;
        // mengakses api untuk mengambil data pelanggaran
        axios.get(url, this.headerConfig())
        .then(response => {
          // mengisikan data dari respon API ke array pelanggaran
          this.setState({detail_pelanggaran_siswa: response.data.detail_pelanggaran_siswa});
          this.setState({id_pelanggarannya_siswa: id_pelanggarannya_siswa, isModal1Open:true})
        })
        .catch(error => {
          console.log(error);
        });
    ...
    savePelanggaran = (event) => {
        ...
        let id = this.state.id_pelanggarannya_siswa
        let form = {
          id_pelanggarannya_siswa: id,
    ...
    dropPelanggaran = (item) => {
        ...
        let url = "http://localhost:2000/detail_pelanggaran_siswa/" + item.id_pelanggarannya_siswa + "/" + item.id_pelanggaran;
        ...
        if (window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
          ...
          .then(response => {
            ...
            this.handleDetail(item.id_pelanggarannya_siswa);
          })
          .catch(error => {
            console.log(error);
          });
    ...
    handleSave = (event) => {  
        event.preventDefault();  
        let url = "http://localhost:2000/pelanggarannya_siswa/save";  
    ...
    handleDrop = (id_pelanggarannya_siswa) => {
        let url = "http://localhost:2000/pelanggarannya_siswa/" + id_pelanggarannya_siswa;
        // memanggil url API untuk menghapus data pada database
        if (window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
            axios.delete(url, this.headerConfig())
            .then(response => {
            ...
            <>
                ...
                <Card.Body>
                    ...
                    <Table striped bordered hover>
                        ...
                        <tbody>
                        { this.state.pelanggarannya_siswa.map(item => {  
                        return(  
                            <tr key={item.id_pelanggarannya_siswa}>  
                            ...
                            <td>
                                ...
                                onClick={() => this.handleDetail(item.id_pelanggarannya_siswa)}> Detail </Button>
                                <Button className="btn btn-sm btn-danger m-1" 
                                onClick={() => this.handleDrop(item.id_pelanggarannya_siswa)}> Hapus </Button> 
                            </td>  
                            </tr>  
                        );  
                        }) }
                        ...
                        <Table striped bordered hover>
                            ...
                            <tbody>
                                { this.state.detail_pelanggaran_siswa.map(it => {  
                                    return(  
                                    <tr key={it.id_pelanggarannya_siswa}>
...
export default PelanggarannyaSiswa 